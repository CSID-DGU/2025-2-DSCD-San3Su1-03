# core/geocode.py (신규)
import os, time, json, requests
from datetime import datetime, timedelta

KAKAO_KEY = os.getenv("KAKAO_REST_API_KEY")  # 또는 st.secrets["kakao"]["rest_key"]

def kakao_reverse(lat: float, lon: float, timeout=6):
    h = {"Authorization": f"KakaoAK {KAKAO_KEY}"}
    p = {"x": str(lon), "y": str(lat), "input_coord":"WGS84"}
    r = requests.get("https://dapi.kakao.com/v2/local/geo/coord2address.json", headers=h, params=p, timeout=timeout)
    if r.status_code == 200:
        docs = r.json().get("documents", [])
        if docs:
            a = docs[0].get("address") or {}
            raddr = docs[0].get("road_address") or {}
            return {
                "sido": a.get("region_1depth_name"),
                "sigungu": a.get("region_2depth_name"),
                "eup_myeon_dong": a.get("region_3depth_name"),
                "road_address": raddr.get("address_name"),
                "address_name": a.get("address_name") or raddr.get("address_name"),
                "provider": "kakao",
            }
    # 폴백: regioncode
    r2 = requests.get("https://dapi.kakao.com/v2/local/geo/coord2regioncode.json", headers=h, params=p, timeout=timeout)
    if r2.status_code == 200:
        docs = r2.json().get("documents", [])
        if docs:
            d = docs[0]
            return {
                "sido": d.get("region_1depth_name"),
                "sigungu": d.get("region_2depth_name"),
                "eup_myeon_dong": d.get("region_3depth_name"),
                "road_address": None,
                "address_name": d.get("address_name"),
                "provider": "kakao-region",
            }
    return None
